...
class Trainer(object):
    ...
    def take_action(self, info, env, brain_name, steps, normalize):
        ...
        if self.use_states:
            #decorelate_states(info.states)
            #print ( "Orig {0}".format (info.states))
            # feature extraction
            #Y = tf.svd(tf.matmul(tf.transpose(info.states, [1,0]), info.states), compute_uv=False).eval()
            #Z = np.empty( info.states.shape, int)
            #np.append(Z[0], Y, axis=0)
            #print ( "Z {0}".format (Z))
